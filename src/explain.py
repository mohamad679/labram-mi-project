...
This module computes CLS-token attention rollout across all LaBraM transformer
blocks and summarizes token importance into:

1. token_importance          : 256 patch-token scores
2. channel_importance_128    : 128 canonical LaBraM channel scores
3. time_patch_importance     : 2 temporal patch scores

The model used here is the Phase 1 head-only checkpoint.
"""
...
"""
Reproduce LaBraM.forward_features up to, but not including, transformer blocks.

```
For this project:
  input x              : (B, 64, 400)
  after interpolation  : (B, 128, 400)
  after patch_embed    : (B, 256, 200)
  after CLS concat     : (B, 257, 200)

Returns
-------
tokens:
    Embedded tokens before transformer blocks, shape (B, 257, 200).
n_input_chans:
    Number of canonical LaBraM channels after interpolation, expected 128.
n_time_patches:
    Number of temporal patches per channel, expected 2.
"""
...
----------
model:
    Loaded LaBraM model with Phase 1 checkpoint weights.
x:
    EEG batch, shape (B, 64, 400).

Returns
-------
dict with:
  rollout:
    Full rollout matrix, shape (B, 257, 257).
  token_importance:
    CLS-to-patch-token scores, shape (B, 256).
  channel_importance_128:
    Mean importance over temporal patches, shape (B, 128).
  time_patch_importance:
    Mean importance over channels, shape (B, 2).
"""
model.eval()
x = x.to(DEVICE)
...
with torch.no_grad():
    ...
    for block in model.blocks:
        attn = block(h, return_attention=True)  # (B, heads, N, N)
...
def top_k_channels(channel_scores: np.ndarray, k: int = TOP_K_CHANNELS) -> list[list[tuple[str, float]]]:
"""
...
"""
Save simple bar plots for top channels and temporal patch importance.
"""
...
for i in range(channel_scores.shape[0]):
    ...
    plt.title(
        f"Sample {sample_indices[i]} | temporal patch importance"
    )
    plt.tight_layout()
